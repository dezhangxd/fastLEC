...
#define USER_PARAMS                                                            \
    ...
    USER_PARAM(ls_bv_bits,                                                     \
               int,                                                            \
               17,                                                             \
               "bitvector width in log scale for logic synthesis")             \
    USER_PARAM(es_bv_bits,                                                     \
               int,                                                            \
               14,                                                             \
               "bitvector width in log scale for para/seq simulation")         \
    USER_PARAM(use_ies, bool, true, "Enable iES")                              \
    USER_PARAM(use_pes_pbit, bool, false, "Enable para-bits for para-es")      \
    USER_PARAM(ies_u64,                                                        \
               bool,                                                           \
               false,                                                          \
               "Enable iES with u64_int, default using long BV for ies")       \
    ...
    USER_PARAM(gt_max_split, int, 2, "Max split times for a task")             \
    USER_PARAM(                                                                \
...
struct CustomParams
{
#define USER_PARAM(name, type, default_val, description)                       \
...
#undef USER_PARAM
        ...
        fprintf(stderr,
                "c [parser] error: unknown custom parameter: %s\n",
                name.c_str());
        help(2);
        exit(1);
    }
...
private:
    ...
    template <typename T> void printDefaultValue(const T &default_val)
    {
        if constexpr (std::is_same<T, bool>::value)
        {
    ...
    template <typename T> void printParamValue(const T &value)
    {
        if constexpr (std::is_same<T, bool>::value)
        {
...
public:
    template <typename T> T getCustomParam(const std::string &name) const
    {
#define USER_PARAM(param_name, type, default_val, description)                 \
    if (name == #param_name)                                                   \
    {                                                                          \
...
#undef USER_PARAM
    ...
    void help(unsigned show = 3)
    {
        if (show == 1 || show == 3)
        {
            printf("c fastLEC: A Fast and Scalable LEC prover for AIGER "
                   "Circuits (version 0.1)\n");
            printf("c Usage: fastLEC -i <input.aig> [options]\n");
            printf("c Options:\n");
            printf(
                "c   -h, --help                show this help information\n");
            printf("c   --modes                   show available modes\n");
            printf(
                "c   -m, --mode <mode>         set the mode [default: ES]\n");
            printf("c   -i, --input <filename>    set the input file\n");
            printf("c   -c, --cores <num>         set the number of threads "
                   "[default: 1]\n");
            printf("c   -t, --timeout <time>      set the timeout [default: "
                   "30.0 seconds]\n");
            printf("c   -v, --verbose <level>     set verbose level [default: "
                   "1]\n");
...
#undef X
            ...
            else if (arg == "-p" || arg == "--param")
            {
                ...
                {
                    fprintf(stderr,
                            "c [parser] error: -p requires two arguments: "
                            "<key> <value>\n");
                    p.help(2);
                    return false;
                }
            ...
            {
                fprintf(
                    stderr, "c [parser] error: unknown option: %s\n", argv[i]);
                p.help(1);
                return false;
            }
        }
