...
public:
    ...
    static const int unit_width =
        8 * sizeof(bv_unit_t); // 64 bit for an uint64_t element
...
template <> struct hash<fastLEC::BitVector>
{
...
template <> struct equal_to<fastLEC::BitVector>
{
    bool operator()(const fastLEC::BitVector &lhs,
                    const fastLEC::BitVector &rhs) const
    {
        return lhs == rhs;
    }
};
